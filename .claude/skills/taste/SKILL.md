...
## Local adaptation (this repo)

Vendored from https://github.com/senlindesign/taste-skill (MIT). Three things
differ here, because the published skill assumes a hosted site and the Playwright
MCP server:

- **No MCP needed.** `scripts/capture.mjs` runs `references/extract.js` on this
  machine's Playwright and Chromium. It was only ever the transport; the extract
  is a plain expression that evaluates the same either way.

      PLAYWRIGHT_MODULES=<node_modules with playwright-core> \
        node .claude/skills/taste/scripts/capture.mjs <url> [outfile]

- **file:// targets work**, which matters because the surface analysed here is a
  single-file app rather than a hosted page. There is no hostname, so outputs are
  named for the project (`taxapp`) instead of a domain.
- **Outputs live in `docs/taste/`**, not the repo root, and the Claude Code export
  target writes to `CLAUDE.md` as the skill specifies.

The published Prerequisites section below still describes the MCP route; use it
if you point this at a real website from a session that has the MCP server.
